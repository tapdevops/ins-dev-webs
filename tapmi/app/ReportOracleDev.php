...
class ReportOracleDev extends Model {
			...
			FROM
				(
					...
					FROM
						...
						LEFT JOIN TAP_DW.TM_EMPLOYEE_HRIS@DWH_LINK HRIS ON
						...
						LEFT JOIN (
							...
							AND LAND_USE.LAND_USE_CODE = EBCC_HEADER.BLOCK_CODE
						LEFT JOIN (
							SELECT * FROM (
								SELECT
	...
	public function EBCC_COMPARE( $REPORT_TYPE , $START_DATE , $END_DATE , $REGION_CODE , $COMP_CODE , $BA_CODE , $AFD_CODE , $BLOCK_CODE ) {
		...
		if ( !empty( $get ) ) {
			...
			foreach ( $get as $ec ) {
				...
				$sql = "SELECT MOBILE_INSPECTION.F_GET_EBCC_COMPARE( '{$ec->val_werks}', '{$ec->val_afd_code}', '{$ec->val_block_code}', '{$ec->val_tph_code}', '{$date}','{$date}' ) AS NO_BCC FROM DUAL";
	...
	public function EBCC_COMPARE_VIEW( $EBCC_VALIDATION_CODE ) {
		$get = $this->db_mobile_ins->select("
				SELECT
					KUALITAS.ID_KUALITAS,
					KUALITAS.NAMA_KUALITAS
